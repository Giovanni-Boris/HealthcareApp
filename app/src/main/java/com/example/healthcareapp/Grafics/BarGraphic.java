...
public class BarGraphic extends View {
    ...
    public BarGraphic(Context context, DisplayMetrics metrics, ArrayList<Double> data) {
        super(context);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int registers = 7;
        int distancia_reg = ancho_disponible / registers+11;
        for (int i = 0; i < registers; i++) {
            canvas.drawText("Reg  " + (i + 1), 15+margen_x + distancia_reg * i, distancia_lineas * cantidad_lineas + margen_y, pencilText);
        }
        canvas.drawText("N* registro ", 15+margen_x + ancho_disponible/2, distancia_lineas * (cantidad_lineas + 1)+ margen_y, pencilText);
        int espacio_central_texto_reg = 25;
        ...
        for (int i = 0; i < registers; i++) {
            float x = i * distancia_reg + margen_x + espacio_central_texto_reg;
            float y = 0;
